...
namespace logger {
    ...
    public:
        ...
        void log(std::string msg) {
            ...
            string_builder << "Log[" << std::put_time(std::localtime(&result), "%Y-%m-%d %H:%M:%S") << "]: " << msg << "\n";
            sink_->write(string_builder.str());
        }

        std::string convert_to_string(const char* t) {
            std::string s{t};
            return s;
        }

        std::string convert_to_string(std::string t) {
            std::string s{t};
            return s;
        }
